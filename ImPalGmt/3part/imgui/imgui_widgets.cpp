...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	RenderFrame(frame_bb.Min, frame_bb.Max, IM_COL32(15, 15, 16,100), false, 9.0f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size, 0));
	
	const ImVec2 arrow_size = CalcTextSize(label, NULL, true);
    const ImRect clickable(window->DC.CursorPos + ImVec2(size-arrow_size.x-size/3.5, -5), window->DC.CursorPos + ImVec2(size, 25));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect total_bb({ window->DC.CursorPos.x+10,window->DC.CursorPos .y}, window->DC.CursorPos + ImVec2(w, label_size.y + 16));
	...
	window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, IM_COL32(15, 15, 16,100), 5.0f);
	window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y), ImColor(147, 190, 66), 5.0f); //»¬¿éÌõ
